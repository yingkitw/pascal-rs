...
pub struct Parser<'a> {
    lexer: Lexer<'a>,
    peeked: Option<Result<(usize, Token, usize), crate::lexer::LexerError>>,
...
impl<'a> Parser<'a> {
    ...
    fn next_token(&mut self) -> Option<Result<(usize, Token, usize), crate::lexer::LexerError>> {
    ...
    fn peek_token(&mut self) -> Option<Result<(usize, Token, usize), crate::lexer::LexerError>> {
    ...
    fn skip_whitespace_and_comments(&mut self) {
        while let Some(Ok((_, Token::Whitespace | Token::Comment, _))) = self.peek_token() {
            self.next_token();
        }
    }
    ...
    fn parse_with_statement(&mut self) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt::With { record: Expr::Set(records), body })
    }
    ...
    fn expect_token(&mut self, expected: Token) -> Result<(), ParseError> {
        ...
        match self.next_token() {
            ...
            Some(Ok((pos, found, _))) => {
                if matches!(found, Token::Whitespace | Token::Comment) {
