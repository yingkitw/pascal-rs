...
impl SymbolTable {
    ...
    pub fn add_symbol(&mut self, name: String, typ: Type, is_parameter: bool, is_exported: bool) -> Result<i32> {
        ...
        if self.scopes[self.current_scope].symbols.contains_key(&name) {
            return Err(anyhow!("Symbol '{}' already defined in current scope", name));
        }
...
mod tests {
    ...
    fn test_symbol_table_basic() {
        ...
        // Add a variable
        let offset = table.add_symbol("x".to_string(), Type::Integer, false, false).unwrap();
        assert_eq!(offset, 8);
    ...
    fn test_scopes() {
        ...
        // Add to global scope
        table.add_symbol("x".to_string(), Type::Integer, false, false).unwrap();
        
        // Enter new scope
        table.enter_scope();
        table.add_symbol("y".to_string(), Type::Integer, false, false).unwrap();
    ...
    fn test_constants() {
        ...
        table.add_const("PI".to_string(), Type::Real, ConstValue::Real(3.14159)).unwrap();
