...
use minipas_ast::{Expr, Stmt, Literal, BinaryOp, FunctionDecl, ProcedureDecl};
...
impl LoopOptimizer {
    ...
    pub fn optimize_loop(&self, stmt: &Stmt) -> Stmt {
        match stmt {
            Stmt::For { var_name, start, end, body, direction } => {
                // Check if loop can be unrolled
                if let (Expr::Literal(Literal::Integer(s)), Expr::Literal(Literal::Integer(e))) = (start, end) {
    ...
    fn substitute_var(&self, stmt: &Stmt, var_name: &str, value: i64) -> Stmt {
        match stmt {
            Stmt::Assignment { target, value: expr } => {
                Stmt::Assignment {
...
mod tests {
    ...
    fn test_loop_unrolling() {
        ...
        let loop_stmt = Stmt::For {
            ...
            direction: minipas_ast::ForDirection::To,
