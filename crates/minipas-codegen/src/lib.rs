...
impl CodeGenerator {
    ...
    fn generate_single_statement(&mut self, stmt: &Stmt) -> Result<()> {
        match stmt {
            ...
            Stmt::With { expr, body } => self.generate_with_statement(expr, body)?,
            ...
            Stmt::Try { body, except_arms, finally_body } => {
                ...
                self.emit(&format!("{}:", try_label));
                self.generate_statement(body)?;
                self.emit(&format!("    jmp {}", end_label));

                // Exception handling would go here
                for arm in except_arms {
            ...
            Stmt::Raise { exception_type, message } => {
                ...
                self.emit("    ; raise exception - simplified");
            }
            ...
            Stmt::TryExcept { body, except_arms } => {
                // Simplified try-except implementation
                let try_label = self.new_label("try");
                let end_label = self.new_label("try_end");
            ...
            Stmt::TryFinally { body, finally_body } => {
                // Simplified try-finally implementation
                let try_label = self.new_label("try");
                let end_label = self.new_label("try_end");
                ...
                // Finally block
                self.generate_statement(finally_body)?;
    ...
    fn type_size(&self, typ: &Type) -> usize {
        match typ {
            ...
            Type::Enum { .. } => 4,  // Default size for enums
            Type::Set { .. } => 256, // Default size for sets (bitset)
            Type::Range { .. } => 8, // Min and max values
            Type::File { element_type: _ } => 4,     // File handle size
            Type::Variant => 4,    // Default size for variant types
            Type::Class { .. } => 8, // Object reference size
            Type::Interface { .. } => 8, // Interface reference size
            Type::VariantRecord { .. } => 8, // Variant record size
            Type::DynamicArray { .. } => 8, // Dynamic array reference size
            Type::OpenArray { .. } => 8, // Open array reference size
        }
    }
    ...
    fn generate_literal(&mut self, lit: &Literal) -> Result<()> {
        match lit {
            ...
            Literal::Set(elements) => {
                return Err(anyhow!("Set literals not implemented yet"));
            }
            Literal::Enum(_, _) => {
                return Err(anyhow!("Enum literals not implemented yet"));
            }
            Literal::Range(_, _) => {
                return Err(anyhow!("Range literals not implemented yet"));
            }
        }
        Ok(())
    }
    ...
    fn generate_expression(&mut self, expr: &Expr) -> Result<()> {
        match expr {
            ...
            Expr::In { expr, set_expr } => {
                self.generate_expression(expr)?;
                self.emit("    push rax");
                self.generate_expression(set_expr)?;
                self.emit("    pop rbx");
                // Simplified 'in' check - just compare values
                self.emit("    cmp rbx, rax");
                self.emit("    setb al");
                self.emit("    movzx eax, al");
            }
            Expr::Is { expr, type_name: _ } => {
                // Type checking - simplified implementation
                self.generate_expression(expr)?;
                self.emit("    mov eax, 1"); // Assume it's always the expected type
            }
            Expr::As { expr, type_name: _ } => {
                // Type casting - simplified implementation
                self.generate_expression(expr)?;
                // For now, just return the expression as-is
            }
            Expr::Range { start, end } => {
                // Range expression - simplified implementation
                self.generate_expression(start)?;
                self.emit("    push rax");
                self.generate_expression(end)?;
                self.emit("    pop rbx");
                // Store min and max in a structure
                self.emit("    ; range expression - simplified");
            }
            Expr::With { expr, body } => {
            ...
            Expr::MethodCall { object, method, args } => {
                ...
                self.emit(&format!("    ; call method {}", method));
            }
            Expr::PropertyAccess { object, property } => {
                self.generate_expression(object)?;
                self.emit(&format!("    ; access property {}", property));
            }
            Expr::PropertyAssignment { object, property, value } => {
                self.generate_expression(object)?;
                self.generate_expression(value)?;
                self.emit(&format!("    ; assign property {}", property));
            }
            Expr::New { type_name, args } => {
                for arg in args {
                    self.generate_expression(arg)?;
                }
                self.emit(&format!("    ; new {}", type_name));
            }
            Expr::Dispose { expr } => {
                self.generate_expression(expr)?;
                self.emit("    ; dispose");
            ...
            Expr::GenericSpecialization { generic_name, type_arguments } => {
                for arg in type_arguments {
                    self.emit(&format!("    ; type arg: {}", arg));
                }
                self.emit(&format!("    ; specialize {}", generic_name));
            }
            Expr::InlineAssembly { code, inputs, outputs, clobbers } => {
                for (reg, expr) in inputs {
                    self.generate_expression(expr)?;
                    self.emit(&format!("    ; input {} -> {}", expr, reg));
                }
                self.emit(&format!("    ; asm: {}", code));
                for (reg, constraint) in outputs {
                    self.emit(&format!("    ; output {} ({})", reg, constraint));
                }
                for clobber in clobbers {
                    self.emit(&format!("    ; clobber {}", clobber));
                }
            }
        }
...
// Implement the trait interfaces
impl CodeGeneratorCapability for CodeGenerator {
    fn new() -> Self {
        CodeGenerator::new()
    }
    ...
    fn generate_statement(&mut self, stmt: &Stmt) -> Result<String> {
        ...
        temp_gen.generate_statement(&[stmt.clone()])?;
        Ok(temp_gen.output)
    }
...
impl AssemblyOutput for CodeGenerator {
    fn emit(&mut self, code: &str) {
...
impl VariableManager for CodeGenerator {
    fn allocate_variable(&mut self, name: &str, typ: &Type) -> Result<i32> {
        // Allocate a new variable and return its stack offset
        let stack_offset = -(self.variables.len() as i32 + 1) * 8; // Simple stack allocation
        let var_info = VariableInfo {
            typ: typ.clone(),
            offset: stack_offset,
            is_param: false,
        };
        self.variables.insert(vec![name.to_string()], var_info);
        Ok(stack_offset)
    }
    
    fn get_variable_offset(&self, name: &str) -> Option<i32> {
        // Look up the variable and return its offset
        if let Some(var_info) = self.variables.get(&vec![name.to_string()]) {
            Some(var_info.offset)
        } else {
            None
        }
    }
