...
use minipas_ast::{Expr, Literal, BinaryOp, UnaryOp, Stmt};
use anyhow::Result;
...
impl Optimizer {
    ...
    fn fold_binary_op(&self, op: &BinaryOp, left: &Expr, right: &Expr) -> Option<Expr> {
        match (left, right) {
            ...
            (_, Expr::Literal(Literal::Integer(0))) if matches!(op, BinaryOp::Add | BinaryOp::Subtract) => {
                Some(left.clone())
            }
            (Expr::Literal(Literal::Integer(1)), _) if matches!(op, BinaryOp::Multiply) => {
                Some(right.clone())
            }
            (_, Expr::Literal(Literal::Integer(1))) if matches!(op, BinaryOp::Multiply | BinaryOp::Divide) => {
                Some(left.clone())
            }
            ...
            Expr::Literal(Literal::Integer(val)) => {
                match op {
                    UnaryOp::Minus | UnaryOp::Negate => {
                        Some(Expr::Literal(Literal::Integer(-val)))
                    }
                ...
                if let Expr::Literal(Literal::Boolean(val)) = opt_condition {
                    if val {
                        ...
                        if then_branch.len() == 1 {
                            ...
                            return Some(Stmt::Block(minipas_ast::Block {
                                consts: vec![],
                                types: vec![],
                                vars: vec![],
                                procedures: vec![],
        ...
        while i < instructions.len() {
            ...
            if i + 1 < instructions.len() {
                if inst.trim().starts_with("push") && instructions[i + 1].trim().starts_with("pop") {
                    ...
                    let pop_reg = instructions[i + 1].trim().strip_prefix("pop").unwrap().trim();
                    if push_reg == pop_reg {
                        i += 2;
                        continue;
                    }
